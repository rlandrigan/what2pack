...
<html lang="en" data-bs-theme="dark">
  <head>
	...
	<title>QBranch - Packing List</title>
  <?php 
  ...
  for (var i = 0; i < arrMarkMailin.length; i++) {
   ...
   <?php
	   ...
	   $stmta = $DBcon->prepare("SELECT * FROM qbranch_event where id = '".$params['event']."' ");
	   $stmta->execute();
	   if($stmta->rowCount() > 0)
	   {
		while($row_ex=$stmta->FETCH(PDO::FETCH_ASSOC))
		{
	   $eventName = $row_ex['name'];
	 }}
		 ?>
...
<link rel="stylesheet" href="/css/alt.css?<?php echo  generateRandomString();?>">
  ...
  <body>
		...
		$stmtu = $DBcon->prepare("select * from qbranch_units where id = '".$params['id']."' ");
		 ...
		 <header class="py-3 mb-4 border-bottom d-print-none">
		   <div class="container d-flex flex-wrap justify-content-center">
			 <span class="d-flex align-items-center mb-3 mb-lg-0 me-lg-auto link-body-emphasis text-decoration-none d-print-none">
			<image src="/img/what2packBackInv.png" style="width: 3rem;transform: rotate(-35deg)" class="me-4">
			   <span class="fs-2 title"><?php print($unitName) ?> Packing List for <?php print($eventName) ?> 
			 </span></span>
			 <form class="col-12 col-lg-auto mb-3 mb-lg-0 d-print-none" role="search">
	  ...
	  <form action="/checkinout" method="POST" id="form">
		...
		<div id="qrcode"  class="d-none d-print-block"></div><div class="d-none d-print-block"><h3><?php print($unitName) ?> Packing List for <?php print($eventName) ?> </h3></div>
		<div id="signoff" class="d-none d-print-block mb-5">Recorded By:__________________________</div>
		<script type="text/javascript">
		 var qrcode = new QRCode(document.getElementById("qrcode"), {
			 text:"http://qbranch.scouts13.org/packinglist/<?php print($params['id']); ?>/<?php print($params['event']); ?>",
		width: 90,
		height: 90,
		 css: "width: 90px",
		 colorDark : "#000000",
 ...
 <?php 
 $stmtP = $DBcon->prepare("select * from qbranch_checklist_items where checklistID = '".$params['event']."' AND checkInDate IS NULL ");
	$stmtP->execute();
   $itemList = '';
	if($stmtP->rowCount() > 0)
	{
	 ...
	 {
		 if($itemList == ''){
		 $itemList = $rowP['itemID'];
	 } else {
	$itemList = $itemList.','.$rowP['itemID'];
	 }
	 }
	 }
 ...
 if($itemList == ''){
	 ...
	 echo('no items in list');
		  ...
		  $stmt = $DBcon->prepare("SELECT * FROM qbranch_master WHERE id IN (".$itemList.") and container < 1 ORDER BY labelID ASC ");
		  $stmt->execute();
		  ?>
...
<?php if (!empty($row['notes'])) { print('<br/>'.$row['notes']);} ?> 
  ...
  <?php if($row['type'] == 'container'){?>
	  ...
	  <?php
		 $stmta = $DBcon->prepare("SELECT * FROM qbranch_master WHERE container = '".$row['id']."' and  id IN (".$itemList.") ORDER BY labelID ASC ");
		 $stmta->execute();
		 ?>
		 <?php
		 if($stmta->rowCount() > 0)
	   ...
	   <?php
		  $stmtb = $DBcon->prepare("SELECT * FROM qbranch_master WHERE container = '".$rowa['id']."' and  id IN (".$itemList.") ORDER BY labelID ASC ");
		  $stmtb->execute();
		  ?>
		  <?php
		  if($stmtb->rowCount() > 0)
  ...
  <?php } 
		...
		<div class="row py-2 d-print-none">
		 ...
		 ?></div>
		 <div class="card-footer"><img src="/img/qbranch.png" class="mx-auto d-block" style="width: 5em">
			</div>
</div>
  </body>
</html>